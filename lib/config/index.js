...
Config.prototype._updateHarvestConfig = function _updateHarvestConfig(serverConfig, key) {
  ...
  const localHarvestLimits = this.event_harvest_config?.harvest_limits || {}

  const limits = Object.keys(val.harvest_limits).reduce((acc, k) => {
    const serverLimit = val.harvest_limits[k]
    if (harvestConfigValidator.isValidHarvestValue(serverLimit)) {
      const localLimit = localHarvestLimits[k]
      // Use the more restrictive (lower) value when local config is set
      if (harvestConfigValidator.isValidHarvestValue(localLimit)) {
        if (localLimit < serverLimit) {
          logger.info(
            'Using local harvest limit for %s: %s (server sent %s)',
            k,
            localLimit,
            serverLimit
          )
        ...
        logger.info(`Omitting local limit for ${k} due to invalid value ${localLimit}, using server value ${serverLimit}`)
        acc[k] = serverLimit
      }
    } else {
      logger.info(`Omitting server limit for ${k} due to invalid value ${serverLimit}`)
    }
    return acc
  }, {})
